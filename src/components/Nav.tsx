...
function BottomNav() {
  return (
    <nav className="fy-bottom-nav" aria-label="Main navigation">
      {TABS.map((tab) => (
        <NavLink
          key={tab.path}
          to={tab.path}
...
function Sidebar() {
  return (
    <aside className="fy-sidebar">
      <NavLink to="/" className="fy-sidebar-logo" end aria-label="Fly YouTube home">
      ...
      >
        {TABS.map((tab) => (
          <NavLink
            ...
            className={({ isActive }) => `fy-sidebar-item${isActive ? ' is-active' : ''}`}
          >
            {({ isActive }) => (
              <span
