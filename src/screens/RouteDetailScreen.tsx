...
import { getCreatorById, getRouteById } from '../services/index.ts';
...
function RouteDetail({ id }: { id: string | undefined }) {
  ...
  usePageTitle(
    ...
    route ? `${route.dep} to ${route.arr} — ${route.aircraft}, ${route.duration}.` : undefined
  );
  ...
  useEffect(() => {
    ...
    getRouteById(routeId)
      .then((loadedRoute) => {
        ...
        return getCreatorById(loadedRoute.creatorId).then((loadedCreator) => {
          if (!cancelled) setCreator(loadedCreator);
        });
      })
      .catch(() => {
  ...
  if (loading) {
    return (
      <div className="fy-page-enter fy-screen">
        <div
          aria-busy="true"
          style={{ height: 220, background: FY.midnight700, borderRadius: 16, marginBottom: 16 }}
        />
      </div>
    );
  }
